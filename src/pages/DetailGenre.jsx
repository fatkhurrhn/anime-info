...
export default function DetailGenre() {
    ...
    const fetchGenreAnime = async (page = 1) => {
        setLoading(true);
        try {
            const response = await fetch(`https://api-animeinfo.vercel.app/api/genre/${slug}?page=${page}`);
            const result = await response.json();

            if (result.success) {
                setGenreInfo({
                    name: result.data.genre_name || result.data.genre,
                ...
                console.error('Failed to fetch genre anime');
            }
        } catch (error) {
    ...
    // Format genre name untuk display
    const displayGenreName = genreInfo?.name?.replace(/ - Page \d+ of \d+$/, '') || slug;
            ...
            <div className="bg-gradient-to-br from-blue-900 via-blue-800 to-cyan-800 shadow-xl">
                <div className="px-5 pt-8 pb-6">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            <h1 className="text-white text-xl font-bold tracking-tight capitalize">
                                {displayGenreName}
                            </h1>
                            {genreInfo && (
            ...
            <div className="px-4 mt-4">
                {loading ? (
                        ...
                        {!hasNextPage && animeList.length > 0 && (
                            <div className="mt-4 text-center py-3">
                                <p className="text-xs text-gray-500">
                                    ...
                                    Menampilkan {animeList.length} anime • Halaman terakhir
                                </p>
                            </div>
                        )}
                    </>
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-400 mt-3 text-sm">Tidak ada anime dalam genre ini</p>
                    </div>
                )}
            </div>
