...
export default function DetailStudio() {
    ...
    const fetchStudioAnime = async (page = 1) => {
        setLoading(true);
        try {
            // Decode slug untuk URL (misal: zero-g -> Zero-G)
            const studioName = decodeURIComponent(slug);
            const response = await fetch(`https://api-animeinfo.vercel.app/api/studio/${studioName}?page=${page}`);
            const result = await response.json();

            if (result.success) {
                setStudioInfo({
                    name: result.data.studio_name || result.data.studio,
                ...
                console.error('Failed to fetch studio anime');
            }
        } catch (error) {
    ...
    // Format studio name untuk display
    const displayStudioName = studioInfo?.name || slug;
                    ...
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-blue-300 text-xs">Studio</span>
                                {studioInfo && (
            ...
            <div className="px-4 mt-4">
                {loading ? (
                        ...
                        {!hasNextPage && animeList.length > 0 && (
                            <div className="mt-4 text-center py-3">
                                <p className="text-xs text-gray-500">
                                    ...
                                    Menampilkan {animeList.length} anime dari studio {displayStudioName} • Halaman terakhir
                                </p>
                            </div>
                        )}
                    </>
                    ...
                    <div className="text-center py-12 bg-gray-800/30 rounded-xl">
                        ...
                        <p className="text-gray-400 mt-3 text-sm">Tidak ada anime dari studio ini</p>
                    </div>
                )}
            </div>
